...
* functions
  * `child_process.spawn()`
    * spawn (== create & run) subprocesses /
      * 's way == [`popen(3)`](https://man7.org/linux/man-pages/man3/popen.3.html)'s way
        * 👀by default, create 3 pipes BETWEEN parent Node.js process -- & -- spawned subprocess 👀
          * are
            * `stdin`,
            * `stdout`,
            * `stderr`
          * limited & platform-specific capacity
            * if the subprocess writes | `stdout` / > limit & output NOT captured -> subprocess blocks wait for the pipe buffer / accept MORE data
              * if the output will NOT be consumed -> use `{ stdio: 'ignore' }` option
      * asynchronously
        * -> NO block Node.js event loop
    * _Example:_ [here | cjs](examples/childProcess/spawn.cjs) & [here | mjs](examples/childProcess/spawn.mjs)
    * command lookup works -- via --
      * if `env` is | `options` object (== `options.env`) -> performed -- via -- `options.env.PATH` environment variable
        * if `options.env.PATH` NOT set ->
          * | Unix, -- via -- default search path of `/usr/bin:/bin`
            * see your OS's manual -- for -- execvpe/execvp
          * | Windows, -- via -- `process.env.PATH`
      * otherwise, -- via -- `process.env.PATH`
  * [`child_process.spawnSync()`][]
    * == `child_process.spawn()` BUT synchronous
      * -> blocks the event loop TILL the spawned process exits OR is terminated
  * REST
    * 👀implemented | PREVIOUS ones 👀
    * [`child_process.exec()`][]
      * spawns a shell +
      * runs a command | that shell /
        * | it's complete -> pass the `stdout` and `stderr` | callback function
    * [`child_process.execFile()`][]
      * == [`child_process.exec()`][] BUT NOT spawn a shell
    * [`child_process.fork()`][]
      * spawns a NEW Node.js process +
      * invokes a specified module / has IPC communication channel /
        * allows
          * sending messages BETWEEN parent -- & -- child
    * [`child_process.execSync()`][]
      * == synchronous version of [`child_process.exec()`][] /
        * block the Node.js event loop
    * [`child_process.execFileSync()`][]
      * == synchronous version of [`child_process.execFile()`][] /
        * block the Node.js event loop
...
* about environment variables,
  * | Windows,
    * case-insensitive
  * | Node.js,
    * sorts environment variable's keys lexicographically
    * | subprocess,
      * uses ONLY the FIRST one / matches case-insensitively

* use cases
  * [synchronous counterparts][],
    * automate shell scripts
  * synchronous methods
    * can impact NEGATIVELY | performance
      * Reason: 🧠stalling the event loop TILL spawned processes complete🧠
...
* EXISTING methods
  ...
  * [`child_process.fork()`][],
  * [`child_process.exec()`][],
    * allow specifying for OPTIONAL `callback` function /
      * | child process terminates, it's invoked
  * [`child_process.execFile()`][]
    * allow specifying for OPTIONAL `callback` function /
      * | child process terminates, it's invoked
* EACH method
  * returns a [`ChildProcess`][] instance
  * implement the Node.js [`EventEmitter`][] API,
    * -> | life cycle of the child process,
      * if some events occur -> parent process can -- register -- listener functions
...
* spawns a NEW process /
  * use the
    * given`command` + CL arguments (| `args`)
  * 's title
    * | macOS, Linux,
      * `argv[0]`'s vaue
    * | Windows, SunOS,
      * `command`
  * `process.argv[0]`
    * | startup, overwritten -- by -- `process.execPath`
      * -> != `argv0` parameter / passed to `spawn` -- from the -- parent
* `command`
  * `string`
  * == command -- to -- run
* `args`
...
* `options` {Object}
  * `cwd`
    * `string|URL`
    * == working directory | process is spawned
      * if path does NOT exist -> child process
        * emits an `ENOENT` error
        * exits IMMEDIATELY
  ...
  * `env`
    * Object
      * == environment key-value pairs /
        * visible | NEW process
      * if `undefined` -> `env` will be ignored
    * by default, `process.env`
  * `argv0`
    * {string}
    * `argv[0]`'s value / sent to the child process
  ...
  * `detached`
    * {boolean}
    * Prepare child process to run independently of its parent process
  ...
  * `serialization`
    ...
    * Possible values are `'json'` and `'advanced'`
      * See [Advanced serialization][] for more details
    * by default, `'json'`
  * `shell`
    * `boolean|string`
      * if you want DIFFERENT shell -> specify it -- via -- string
    * if `true` -> runs `command` | shell
      * ❌NOT pass unsanitized user input | this function❌
        * Reason: 🧠ANY input / contain shell metacharacters may -- be used to -- trigger
          arbitrary command execution🧠
    * |
      * Unix
        * use `'/bin/sh'`
      * Windows,
        * use `process.env.ComSpec`
    * see
      * [Shell requirements][]
