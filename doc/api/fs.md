...
* `mode`
  * `integer`
  * OPTIONAL
  * modifiers / specify the copy operation's behaviour
  * you can create a mask / consist of the bitwise OR >=2 values
    * _Example:_ `fs.constants.COPYFILE_EXCL | fs.constants.COPYFILE_FICLONE`
  * by default, `0`
...
* | destination file has been opened for writing & an error occurs -> attempt
...
* `src`
  * `string|Buffer|URL`
  * source filename / is copied
* `dest`
  * `string|Buffer|URL`
  * copy operation's destination filename
  * | open a destination file for writing, if an error occurs after -> Node.js will -- attempt to -- remove the destination
* `mode`
  * `integer` & OPTIONAL
  * copy operation modifiers
  * by default, `0`
* `callback(Error)`
