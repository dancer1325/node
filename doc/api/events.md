...
* 💡Node.js core API -- is built around -- an asynchronous event-driven architecture💡
  * emitters
    * := objects / -- emit -- named events
      * == ALL objects / emit events -> `EventEmitter` class' instances
    * `.on()`
      * == function / allows >=1 functions -- are attached to -- named events / emitted by the object
        * event names
          * valid ANY JavaScript property key
          * recommendations
            * camel-cased strings
    * _Examples:_
      * [`net.Server`][]
        * == object / if a peer connects | it -> emits an event
      * [`fs.ReadStream`][]
        * == object / if a file is opened -> emits an event
      * [stream][]
        * == object / if data is available to be read -> emits an event
  * listeners
    * == `Function` objects / -- are called by -- emitters

When the `EventEmitter` object emits an event, all of the functions attached
to that specific event are called _synchronously_.
Any values returned by the called listeners are _ignored_ and discarded.

The following example shows a simple `EventEmitter` instance with a single
listener.
The `eventEmitter.on()` method is used to register listeners, while
