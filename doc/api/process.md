...
* returns
  * {string}
  * Node.js's current working directory
...
* {Object} / 👀contains the [user environment](https://github.com/dancer1325/man-pages/blob/master/man7/environ.7) 👀
  * ⚠️if you add it -> ONLY reflected |
    * Node.js process or ⚠️
    * OTHER [`Worker`][] threads ⚠️
      * if you do NOT specify | create a [`Worker`][] instance -> OWN copy of `process.env` /
        * EACH [`Worker`][] thread
          * == changes | `process.env` -> NOT visible across [`Worker`][] threads
          * changes | MAIN [`Worker`][] thread's `process.env` -> visible | OS or native add-ons
        * -- based on -- its parent thread's `process.env`
    * ❌NOT, if you run `node -e 'process.env.foo = "bar"' && echo $foo` ❌
  * 👀if you want to delete an entry -> `delete process.env.key` 👀
  * | Windows, case-insensitive
    * EXCEPT to children [`Worker`][] instances
* _Example:_
  ```js
  {
    TERM: 'xterm-256color',
