...
## Loading ECMAScript modules -- via -- `require()`
...
* see [Determining module system][]

* `require()`
  * 👀ONLY load ECMAScript modules / meet the following requirements 👀
    * module is FULLY synchronous
      * == NO top-level `await`
    * 1 of these conditions are met
      1. `.mjs` extension
      2. `.js` extension & the closest `package.json` contains `"type": "module"`
      3. `.js` extension & the closest `package.json` does NOT contain `"type": "commonjs"` & module contains ES module syntax
  * return [Module Namespace Object][]
  * vs `import()`
    * synchronous

* _Example:_ see [here](samples/modules), with
  * ES modules [point](samples/modules/point.mjs), [distance](samples/modules/distance.mjs)
  * CJS module [require](samples/modules/require.cjs)
  * `node require.cjs`
    * Problems:
      * Problem1: "Error [ERR_REQUIRE_ESM]: require() of ES Module .. not supported"
        * Suggestion: replace `require()` -- by -- `import()`
        * Solution: TODO:
...
ES Modules.
If the namespace already defines `__esModule`, this would not be added.
This property is experimental and can change in the future.
It should only be used
by tools converting ES modules into CommonJS modules, following existing ecosystem
conventions.
Code authored directly in CommonJS should avoid depending on it.
...
* see [Modules: `module` core module](module.md#source-map-v3-support)
